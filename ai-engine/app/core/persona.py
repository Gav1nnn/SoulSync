...
def generate_with_persona(persona: PersonaProfile, character_name: str, user_message: str, rag) -> str:
    """用结构化 persona 构造 system prompt，再调用 LLM 生成回复。"""
    messages = [
        {"role": "system", "content": build_system_prompt(persona, character_name),},
        {"role": "user"  , "content": user_message},
    ]
    #return chat_completion(messages)
    return rag._ask_llm(messages)